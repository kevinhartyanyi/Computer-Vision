...
{
public:
	Player(Point start, Point end) : Drawable(start, end), length(end.x - start.x) {};
	Point getStart() const { return _start; }
	Point getEnd() const { return _end; }
	void draw() override {
		rectangle(image, _start, _end, BLACK, CV_FILLED);
	...
	void changeStartPosition(int start) {
		if (start > 0) {
