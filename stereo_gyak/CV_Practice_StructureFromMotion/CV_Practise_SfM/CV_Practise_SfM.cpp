...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	// Load images
	cv::Mat image1 = cv::imread("assets/0000.jpg", cv::IMREAD_ANYCOLOR);
	cv::Mat image2 = cv::imread("assets/0001.jpg", cv::IMREAD_ANYCOLOR);
	...
	// Calibration matrix
	start = std::chrono::system_clock::now();
	cv::Mat K = (cv::Mat_<double>(3, 3) << 2759.48, 0, 1520.69,
		0, 2764.16, 1006.81,
		0, 0, 1);
	...
	std::vector<cv::Point2d> &destination_points_)
{
	...
	cv::Ptr<cv::xfeatures2d::SIFT> detector = cv::xfeatures2d::SIFT::create(8000); // The SIFT detector
	...
	/*for (int i = 0; i < keypoints1.size(); ++i)
	{
		...
		{
			double distance =
				cv::norm(descriptors1.row(i) - descriptors2.row(j));
